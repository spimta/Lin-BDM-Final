...
def main(sc):
    ...
    df_main = df_median.alias('a').join(df_stddev,
                ...
                .withColumn('low', udfLow('median', 'stddev')).withColumn('high', df_main.median + df_main.stddev).drop(df_main.stddev).withColumn('year', F.year(df_main.date)) \
                .withColumn("date", udfChangeYear('date'))
